...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file) && move_uploaded_file($_FILES["fileToUpload_id"]["tmp_name"], $target_file_id)) 
    {
     ...
     $result2=mysqli_query($conn,"select * from student");
     $c=mysqli_num_rows($result2);
     $num=$c + 1;
     $bid="CS100".$num;
     $pass=$bid."123";
     $sql="INSERT INTO student(student_idgen,student_name,student_address,student_dob,student_gender,student_email,password,student_phone,  id_proof,id_proof_photo,student_photo,student_status,doj,batch_id,  status, updated_at,created_at) VALUES('$bid','$Name','$address','$dob','$gender','$Email','$pass','$Mobileno','$idproof','$image_id','$image','$studentstatus','$joindate','$batchid','1','".date('Y-m-d')."','".date('Y-m-d')."')";
     $sql2="INSERT INTO user_login(user_id,password,role,status) VALUES('$Email','$pass','student','active')";
